...
int clock = 0; //global time

typedef struct {
    int to, from, cnt, time, run;
    struct list_head mg_q;
} MG;
LIST_HEAD(migration_queue);
MG mg = {.mg_q = LIST_HEAD_INIT(mg.mg_q)};
void migration(){
    if(mg.time > 0) {mg.time--; return;}
    if(mg.run){
        rep(_, mg.cnt){
            struct list_head *first = mg.mg_q.next;
            process *front = list_entry(first, process, list);
            list_move_tail(first, &cpu[mg.to].ready_q);
            printf("%04d  CPU%d: Migrate : COMPLETED!\tPID: %03d\n",
                clock, mg.from, front->pid
            );
            cpu[mg.to].ready_q_cnt++;
        }
        mg.run = 0;
    }
    else{
        if(cpu[0].cur == NULL || cpu[1].cur == NULL) return;
        if(cpu[0].cur->pid != IDLE && cpu[1].cur->pid != IDLE) return;
        mg.to = (cpu[0].cur->pid == IDLE) ? 0 : 1;
        mg.from = 1 - mg.to;
        mg.cnt = (cpu[mg.from].ready_q_cnt - 1) / 2;
        mg.time = MG_LATENCY;
        mg.run = 1;

        process *p, *tmp; int cnt = 0;
        list_for_each_entry_safe_reverse(p, tmp, &cpu[mg.from].ready_q, list){
            if(p->pid == IDLE) continue;

            list_move_tail(&p->list, &mg.mg_q);
            cpu[mg.from].ready_q_cnt--;
            if(++cnt == mg.cnt) break;
        }
    }
}
...
/*
두 cpu가 context switching일때는 waiting queueu로 옮김
arrival_time이 낮은 순으로 같다면 pid가 낮으 순으로 정렬
*/
void roading_cpu(){
...
/*
ready_queue에 있는 작업들을 cpu현재 작업으로 옮기는 역할
*/
void dispatch_cpu(){
    for(int id = 0; id < 2; id++){
        CPU *c = &cpu[id];
...
void simulation(){
    for(clock = 0; ;clock++){
        migration();
        context_switching(); //context switching이 끝나는 시점을 관리
        roading_cpu(); //job_queueu에 있는 process를 ready or waiting으로 옮김
        dispatch_cpu(); //ready_queue에 있는 작업들을 cpu현재 작업으로 옮기는 역할
        working_cpu(); //실제 process 작업 시뮬레이션
        if(out()) return; //simulation 종료 조건을 관리
    }
}
...
int main(){
    input_job_queue(); //이진 파일을 job_queue에 input하는 함수
    simulation(); //전체적인 시뮬레이션을 관리
    show_statistics(); //마지막 통계를 출력
    free_memory(); //동적메모리 해제
    return 0;
}
